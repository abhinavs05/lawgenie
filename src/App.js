...
function App() {
  ...
  const handleKeyterms = async () => {
    ...
    if (data?.data) setResults((prev) => ({ ...prev, keyterms: data.data.terms || [] }));
  };
  ...
  const handleCompare = async () => {
    alert("Compare requires two uploaded documents. This feature is a work in progress.");
  };
  ...
  const handleClauses = async () => {
    ...
    if (data?.data) setResults((prev) => ({ ...prev, clauses: data.data.clauses || "" }));
  };
